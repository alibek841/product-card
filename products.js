export const products = [
	{
		id: 1,
		name: "Увлажняющая мусс",
		description: "Глубоко увлажняет кожу лица, оставляя её мягкой и гладкой.",
		category: "для нормальной кожи",	
		price: 2750,
		image: "images/mousse.jpg",
		compounds: ["активные натуральные комплексы", "витамины С, А, РР, В И Е", "солнцезащитные компоненты"]
	},
  {
    id: 2,
    name: "Крем для увлажнения",
    description: "Способствует удерживанию влаги в верхних слоях кожи.",
    category: "для нормальной кожи",
    price: 3500,
    image: "images/gel.jpg",
    compounds: ["воски", "минералы", "масла"]
  },
  {
    id: 3,
    name: "Гель для умывания",
    description: "Интенсивно очищает, не повреждает защитный барьер кожи.",
    category: "для нормальной кожи",
    price: 1650,
    image: "images/gel.jpg",
    compounds: ["минералы", "витамины С, А, РР, В И Е", "солнцезащитные компоненты"]
  },
  {
    id: 4,
    name: "Подарочный набор №1",
    description: "Набор, состоящий из увлажняющего крема и маски.",
    category: "для нормальной кожи",
    price: 4750,
    image: "images/cream&mask.jpg",
    compounds: ["воски", "минералы", "масла"]
  },
  {
    id: 5,
    name: "Подарочный набор №5",
    description: "Весь набор средств Invisible symphony, крем, маска, мусс и гель для умывания.",
    category: "для нормальной кожи",
    price: 7520,
    image: "images/cream&gel&mask&mousse.jpg",
    compounds: ["воски", "минералы", "масла"]
  }
];

